...
def run_preprocess(input_path):
    ...
    in_file = base.joinpath('NIFTI', f'{pid}_T1C.nii.gz')
    out_file = base.joinpath('NIFTI', f'{pid}_T1C_hdbet.nii.gz')
    ...
    for sequence in sequences:
        mask_file = base.joinpath('NIFTI', f'{pid}_brain_segmentation.nii.gz')
        in_file = base.joinpath('NIFTI', f'{pid}_{sequence}_co.nii.gz')
        out_file = base.joinpath('NIFTI', f'{pid}_{sequence}_hdbet.nii.gz')
    ...
    for i, sequence in enumerate(sequences):
        in_file = base.joinpath('NIFTI', f'{pid}_{sequence}_hdbet.nii.gz')
        out_file = base.joinpath('NIFTI', f'{pid}_{sequence}_hdbet_n4.nii.gz')
        brain_mask = str(base.joinpath('NIFTI', f'{pid}_brain_segmentation.nii.gz'))
